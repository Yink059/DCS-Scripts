--[[
 ...
 local mission_table_filepath = lfs.writedir() .. "Alpen/mission.json"
 ...
 ctld.AASystemLimitRED = 4  -- Red side limit
 
 ctld.AASystemLimitBLUE = 4 -- Blue side limit
 ...
 ctld.pickupZones = {
     { "pick-up-zone mozdok", "none", -1, "no",  0 },
     { "pick-up-zone red", "none", -1, "yes", 1 },
     { "pick-up-zone blue", "none", -1, "yes", 2 },
 }
 ...
 local json = io.open(mission_table_filepath, "r")
 local json_string = json:read("*all")
 local mission_table = net.json2lua(json_string)
 
 for coalition, coaTable in next, mission_table.mission.coalition do
         ...
         if countryTable.helicopter ~= nil then
             for groupKey, groupValue in next, countryTable.helicopter.group do
                 for unitKey, unitTable in next, groupValue.units do
                     table.insert(ctld.transportPilotNames, unitTable.name)
                     log.write("CTLD", log.INFO,
                         "added " .. unitTable.name .. " to transport pilots")
                 end
             end
         end
     end
 ...
 ctld.loadableGroupsByType["SA342L"]       = {}
 ctld.loadableGroupsByType["SA342M"]       = {}
 ...
 function ctld.changeRemainingGroupsForPickupZone(_zoneName, _amount)
     ...
     if _triggerZone == nil then
         trigger.action.outText(
             "CTLD.lua ctld.changeRemainingGroupsForPickupZone ERROR: Cant find zone called " .. _zoneName, 10)
         return
     end
 ...
 function ctld.unloadTroops(_args)
     ...
     if not ctld.troopsOnboard(_heli, _troops) then
         ...
         if ctld.inLogisticsZone(_heli) == true or ctld.farEnoughFromLogisticZone(_heli) == false then
             ctld.displayMessageToGroup(_heli, "You can't drop troops here! Take them to where they're needed!", 20)
             return false
         end
 ...
 function ctld.unpackFOBCrates(_crates, _heli)
     ...
     if _totalCrates >= ctld.cratesRequiredForFOB then
         ...
         local _txt = string.format(
             "%s started building FOB using %d FOB crates, it will be finished in %d seconds.\nPosition marked with smoke.",
 ...
 function ctld.inLogisticsZone(_heli)
     ...
     for _, t in pairs(ctld.pickupZones) do
         local trigger = trigger.misc.getZone(t[1])
 
         if t[5] == _heli:getCoalition() then
             --get distance
             local _dist = ctld.getDistance(_heliPoint, trigger.point)
 
             if _dist <= trigger.radius then
                 return true
             end
         end
     end
                     ...
                     if ctld.addedTo[tostring(_groupId)] == nil then
                         ...
                         if _unitActions.troops then
                             ...
                             for _, _loadGroup in pairs(ctld.loadableGroups) do
                                 for _, groupType in next, ctld.loadableGroupsByType[_unit:getTypeName()] do
                                     if _loadGroup.name == groupType then
     ...
     if _enemyUnit == nil then
         ...
         if _enemyUnit ~= nil then
             ...
             ctld.jtacCurrentTargets[_jtacGroupName] = {
                 name = _enemyUnit:getName(),
                 unitType = _enemyUnit:getTypeName(),
                 unitId =
                     _enemyUnit:getID()
             }
